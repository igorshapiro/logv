...
List.prototype.appendItem = function(item, position) {
  var self = this;

  if (position != null) {
    this.ritems.splice(position + 1, 0, item);
  }
  else {
    this.ritems.push(item);
  }

  var content = item.text ? (item.text || "") : item;
  var contentByLine = content.split("\n");
  var contentLines = contentByLine.length;

  for (var i = 0; i < contentByLine.length; i++) {
    var l = contentByLine[i].replace(/\033\[[0-9;]*m/g, "");
    contentLines += Math.ceil(l.length / this.screen.width) - 1;
  }

  totalHeight = this.itop;
  if (this.items.length > 0) {
    lastItem = this.items[position || this.items.length - 1]
    totalHeight += lastItem.top + lastItem.height;
  }
  ...
  if (position != null) {
    ...
    for (var i = position + 2; i < this.items.length; i++) {
      this.items[i].top += contentLines;
    }
  }
  else {
...
List.prototype.removeItem = function(child) {
  ...
  if (~i && this.items[i]) {
    var removedHeight = this.items[i].height;
    ...
    for (var j = i; j < this.items.length; j++) {
      this.items[j].top -= removedHeight;
    }
  }
};
...
List.prototype.select = function(index) {
  ...
  var newSelectedItem = this.items[index];
  var selectedOffset = 0;
  if (index > this.selected) {
    selectedOffset = newSelectedItem.height + newSelectedItem.top + 1;
  }
  else {
    selectedOffset = newSelectedItem.top;
  }

  this.selected = index;
  this.value = this.ritems[this.selected];

  this.scrollTo(selectedOffset);
};
